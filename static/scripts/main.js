...
/***/ function(module, exports, __webpack_require__) {
	...
	var _parseUrl = __webpack_require__(48);

	var _parseUrl2 = _interopRequireDefault(_parseUrl);

	var _historia = __webpack_require__(49);

	var _historia2 = _interopRequireDefault(_historia);
	...
	var pageCache = {}; /* eslint no-unused-vars: "off" */
	...
	function fetchPage(path) {
	  ...
	  return fetch(path).then(function (response) {
	    ...
	    var content = getPageContent(html);
	    if (!content) throw new Error('Could not parse content for ' + path + '.');
	    pageCache[path] = content;
	    return content;
	  }).catch(function (err) {
	    _alert2.default.push('Oh no! There was an error making your request.');
	    throw err;
	  });
	}

	var pageContentFilter = /<main[^>]*>([\s\S]*)<\/main>/;
	function getPageContent(html) {
	  var match = pageContentFilter.exec(html);
	...
	function init() {
	  ...
	  _alert2.default.push('This is a test.');

	  var historia = new _historia2.default(function (state) {
	    return fetchPage(state.path).then(function (content) {
	      return setPageContent(content);
	    }).then(function () {
	      return setScroll(state.hash);
	    }).catch(function (err) {/* We should track these */});
	  });

	  (0, _delegate2.default)(document.body, 'click', 'a[href]', function (e, el) {
	    if (e.ctrlKey || e.metaKey || e.shiftKey || e.altKey || e.which > 1) return;

	    var link = (0, _parseUrl2.default)(el.href);
	    var page = (0, _parseUrl2.default)(location.href);

	    if (el.href === location.href) e.preventDefault();else if (link.origin === page.origin && link.path !== page.path) {
	      e.preventDefault();
...
/***/ function(module, exports) {
	...
	var matches = Element.prototype.matches || Element.prototype.matchesSelector || Element.prototype.mozMatchesSelector || Element.prototype.msMatchesSelector || Element.prototype.oMatchesSelector || Element.prototype.webkitMatchesSelector || function (selector) {
	  var node = this;
	  var matchedNodes = node.parentNode.querySelectorAll(selector);
	...
	var matchClosest = function matchClosest(node, selector) {
	...
	function delegate(ancestor, eventType, selector, callback, useCapture) {
	  function listener(e) {
	    var target = matchClosest(e.target, selector);
...
/***/ function(module, exports) {
	...
	function parseUrl(url) {
	  if (!urlCache[url]) {
	    ...
	    var origin = a.origin;
	    if (a.origin) {
	      var host = a.host.replace(/:(80|443)$/, '');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Historia = function () {
	  function Historia(onChange) {
	    ...
	    window.addEventListener('popstate', function (e) {
	      ...
	      var title = e.state && e.state.title;
	      _this.push({ url: url, title: title, isPopState: true });
	    });
	  }
	  ...
	  _createClass(Historia, [{
	    ...
	    value: function push(_ref) {
	      var url = _ref.url,
	          title = _ref.title,
	          isPopState = _ref.isPopState;

	      var prevState = this.state;
	      var nextState = getState(url, title);
	      this.state = nextState;
	      if (prevState.path === nextState.path) return Promise.resolve(null);
	      return this.onChange(nextState).then(function () {
	...
	function getState(url, title) {
	  var state = (0, _parseUrl2.default)(url);
	...
	var Alert = function () {
	  ...
	  _createClass(Alert, [{
	    ...
	    value: function add(message) {
	      var newAlert = this.template.replace(/%%message%%/, message);
